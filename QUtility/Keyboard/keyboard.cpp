...
keyboard::keyboard(QString str,QWidget *parent) :
...
QString keyboard::GetReturnValue()
{
    return ui->valueEdit->text().trimmed();
}

void keyboard::SetStringRange(int min,int max)
{
    this->m_nMinLength=min;
    this->m_nMaxLength=max;
}

void keyboard::SetChangeMethodEnabled(bool enabled)
{
...
void keyboard::UpdateInputMessage(QString str)
{
    if(str.isEmpty())
    {
        return ;
...
void keyboard::UpdateInputMessage(char c)
{
...
void keyboard::setBarVisible (int sel, bool enable)
{
    switch(sel) {
...
void keyboard::setBarEnabled (int sel, bool enable)
{
    for(int i=0;i<10;i++)
    {
